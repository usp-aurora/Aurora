...
async def main():
	...
	codigos_unidades = {key:value for (key, value) in map(lambda x: (x.string, re.search(r"codcg=(\d+)", x.get('href')).group(1)), links_unidades)}
...
async def iterar_unidades(codigos_unidades):
	...
	unidade_tasks = (await asyncio.wait([asyncio.create_task(iterar_unidade(i)) for i in codigos_unidades]))[0]
	...
	for materias_unidade in unidade_tasks:
		for materia in materias_unidade.result():
			coros.append(asyncio.create_task(parsear_materia(materia)))
...
def parsear_info_materia(tabelas_folha):
	...
	for folha in tabelas_folha:
		...
		if folha.find(string=re_nome): # Cabeçalho
		...
		elif folha.find(string=re_creditos):
...
def parsear_turmas(tabelas_folha):
	...
	for folha in tabelas_folha:
		if folha.find_all(string=re.compile(r"Código\s+da\s+Turma", flags=re.UNICODE)):
			if info != None:
				if not horario:
					logger.warning(f" -      Turma {info['codigo']} não possui horário cadastrado");
				elif not vagas:
					logger.warning(f" -      Turma {info['codigo']} não possui vagas cadastradas");
				else:
					info['horario'] = horario
					info['vagas'] = vagas
					turmas.append(info)
			info = parsear_info_turma(folha)
		elif folha.find_all(string="Horário"):
			horario = parsear_horario(folha)
		elif folha.find_all(string=re.compile(r"Atividades\s+Didáticas", flags=re.UNICODE)):
			continue
		elif folha.find_all(string="Vagas"):
