...
async def main():
	...
	codigos_unidades = {key:value for (key, value) in map(lambda x: (re.search(r"codcg=(\d+)", x.get('href')).group(1),x.string), links_unidades)}
	...
	if not args.unidades:
		cursos = await iterar_unidades(codigos_unidades.keys())
	else:
		cursos = await iterar_unidades(args.unidades)
...
async def iterar_unidades(codigos_unidades):
	...
	for cursos_unidade in unidade_tasks:
		for (curso_link,curso_periodo) in cursos_unidade.result():
			coros.append(asyncio.create_task(parsear_curso(curso_link,curso_periodo)))
...
async def parsear_curso(link,periodo):
	...
	async with semaforo:
		...
		try:
			response = await session.get(link, timeout=args.timeout, ssl=False)
			assert response.status == 200
		...
		curso['nome'] = ' - '.join(x.group(1) for x in re.finditer(r"Curso:\s*(.+)\s*", soup.get_text()))
		curso['unidade'] = codigos_unidades[re.search("codcg=([1-9]+)",link).group(1)]
		
		re_disciplinas = re.compile(r"Disciplinas\s+Obrigatórias")
		for folha in tabelas_folha:
			if folha.find(string=re_disciplinas):
...
def parsear_periodos(folha):
	...
	periodo = ''
	for tr in trs:
		str = re.sub(r"\s+"," ",next(tr.stripped_strings,''))
