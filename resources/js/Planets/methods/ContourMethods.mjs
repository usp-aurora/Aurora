function getIslandContours(positions, faces, adjacency, noise, threshold = 0.5) {
	...
	const stack = faces.map((face, idx) => {
		for(let i=0; i<3; ++i)
			if(threshold < noise[face[i]] && noise[face[(i+1)%3]] <= threshold)
				return [idx, i, true]
	}).filter(entry => entry)
	const islands = []
	while(stack.length != 0) {
		...
		for(let i=0; i<3; ++i) {
			...
			if(noise[face[c]] <= threshold && threshold < noise[face[d]]) {
