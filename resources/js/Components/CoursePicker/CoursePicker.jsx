...
const Filter = styled.div`
  display: flex;  
  max-height: ${({$state}) => ($state ? '500px' : '0')}; /* Define o limite de altura */
  animation:  ${({$state}) => ($state ? slideIn : slideOut)} 1s ease-in-out ${({$state}) => ($state ? '-0.1s' : '-0.6s')};

  width: 100%;
  height: 196px;

  margin-top: ${({$state}) => ($state ? '1%' : '0')};
